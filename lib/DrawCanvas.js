...
export default function (canvas, config) {
	...
	let canvasConfetti = {
		...
		start() {
			...
			Paper.bounds = new Vector2D(this.canvasWidth * window.devicePixelRatio, this.canvasHeight * window.devicePixelRatio);

      for (let i = 0; i < config.PAPER_COUNT; i++) {
				this.confettiPapers[i] = new Paper(
          Math.random() * 400 + (((this.canvasWidth * window.devicePixelRatio) / 2) - 200),
					(Math.random() * -50) - 25,
					config
				);
			}
