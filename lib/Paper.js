 ...
 */
export default function Paper(x, y, config) {

	let angle = config.DEG_TO_RAD * Math.random() * 360;
	let corners = [];
	let randomIndex = config.COLOR_INDEX(config.COLORS);
	...
	return {
		...
		update(duration) {
			...
			if (this.position.y > Paper.bounds.y) {
			   this.position.x = 0;
			   this.position.y = Paper.bounds.y + 100;
			}
		},
